...
module.exports = class Application extends Emitter {
  ...
  listen(...args) {
    debug('listen');
    const server = this.sslOptions.useSSL === true
        ? uWS.SSLApp(this.sslOptions)
        : uWS.App(this.sslOptions);
    return server
        .any(`*`, this.callback())
        .ws(`*`, {
            ...this.websocketDefaults,
            ...
            open ( ws ) {
                // handle auth/whatever
                console.log(`Websocket opened at ${ws.url}`);
            },

            close ( ws ) {
                console.log(`Websocket closed at ${ws.url}`, ws.getRemoteAddressAsText());
            },
        })
        .listen(...args);
  }
